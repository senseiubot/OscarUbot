...
module.exports = welcome = async (nino, anu) => {
		...
		try {
            ...
            nino.sendMessage(anu.jid, 'Halo! Terima Kasih sudah Mengundangku, Jika ingin Menggunakan Bot, Tolong Jadikan Admin Ya, Ketik !menu', 'conversation')
             ...
             if (!welkom.includes(anu.jid)) return
              ...
              buff = await getBuffer(pic)
               nino.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `Welcome Message By ${setting.botName}`, buttons: [{buttonId: `!menu`,buttonText:{displayText: 'Menu Bot'},type:1},{buttonId: `!rules`,buttonText:{displayText:'Rules Bot'},type:1},{buttonId:`!infogrup`,buttonText:{displayText:'Info Group'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		}
            ...
            if (!welkom.includes(anu.jid)) return
               ...
               buff = await getBuffer(pic)
               nino.sendMessage(mdata.id, { contentText: `${out}`, footerText: `Welcome Message By ${setting.botName}`, buttons: [{buttonId: `!welcome disable`,buttonText:{displayText: 'Disable Leave Message'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
            }
		} catch (e) {
